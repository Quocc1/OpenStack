from dagster import job, op, resource, OpExecutionContext
...
@resource(config_schema={"host": str, "port": str, "user": str})
def trino_resource(context: OpExecutionContext):
    connection_params = {
...
def create_schemas_op(context: OpExecutionContext, dependent_job=None):
...
@op(required_resource_keys={'trino'})
def drop_tables_op(context: OpExecutionContext, dependent_job=None):
    trino = context.resources.trino
...
def drop_all_tables_in_schema(context: OpExecutionContext, trino, schema_name) -> None:
    with trino.get_connection() as conn:
        cursor = conn.cursor()
